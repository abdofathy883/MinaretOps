...
namespace Infrastructure.Identity
{
    public class AuthService : IAuthService
    {
        ...
        public async Task<AuthResponseDTO> LoginAsync(LoginDTO login)
        {
            ...
            if (user.IsDeleted)
                return FailResult("هذا الحساب غير موجود");

            //var token = await userManager.GeneratePasswordResetTokenAsync(user);
            //await userManager.ResetPasswordAsync(user, token, "NewPassword123!");

            //var passCheck = await userManager.CheckPasswordAsync(user, "NewPassword123!");
            //var passCheck = await userManager.CheckPasswordAsync(user, login.Password.Trim());
            //if (!passCheck)
            //    return FailResult("لا يوجد حساب بهذه البيانات");
            //var user = await userManager.Users.SingleAsync(u => u.PhoneNumber == login.PhoneNumber);
            //var user = await userManager.FindByNameAsync(login.PhoneNumber);
            //if (user.IsDeleted)
            //{
